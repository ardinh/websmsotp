...
print_r($json);
if($json['stat'] == 6){
    $m = "Terjadi error pada device ".$json['devid']." saat melakukan request ke url : ".$json['url'].". Silahkan cek lebih lanjut link tersebut";
}else if($json['stat'] == 10){
    $m = "Terjadi error pada device ".$json['devid']." saat melakukan request ke url : ".$json['url'].". Segera cek lebih lanjut link tersebut!!!";
}
print_r($m);
sendMessage($m);
die();
...
function sendMessage($c){
    ...
    $chatid = array("834307676","95364085");//ardi,pa helmie
    $pesan  = $c;
